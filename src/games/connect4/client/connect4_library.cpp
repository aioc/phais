...
static int _board_width;
static int _board_height;
static int my_ID;
...
static void handle_move(int sock, const char* args) {
    int playerid, col;
    int res = sscanf(args, "%d %d", &playerid, &col);
    if (res != 2) {
        printf("CLIENT LIBRARY INTERNAL ERROR: "
                "Incorrect arguments on PLAYER.\n");
    }
    alarm(1);
    clientMoveMade(playerid, col);
    alarm(0);
}
